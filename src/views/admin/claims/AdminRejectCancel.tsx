...
import { getRejectedCancelledClaims } from "@/lib/redux/reducers/claims.reducer";
...
const AdminRejectCancel = () => {
  ...
  useEffect(() => {
    dispatch(getRejectedCancelledClaims());
  }, [dispatch]);

  const rejectedCancelClaimsState: Claim[] = useSelector(
    (state: RootState) => state.claim.claims
  );

  const filteredData = useMemo(() => {
    const lowerQuery = searchQuery.toLowerCase();
    return rejectedCancelClaimsState.filter(
      (claim) =>
  ...
  }, [searchQuery, rejectedCancelClaimsState]);
  ...
  const handleRowClick = (claimId?: string) => {
    if (claimId) {
      navigate(`/admin/claims/${claimId}`, {
        state: { from: "/admin/claims/rejected-cancelled" },
      });
    }
  };
    ...
    <div className="px-2 py-4 md:px-4">
      <div className="flex flex-col p-4 rounded-lg bg-white shadow-lg">
        ...
        <div className="mb-4 text-black">
          <h2 className="text-2xl font-bold mb-2">Rejected/Cancelled</h2>
          <p>Here is a list of rejected/cancelled</p>
...
export default withAuthorization(AdminRejectCancel, allowedRoles);

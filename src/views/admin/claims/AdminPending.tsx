...
import { getPendingClaims } from "@/lib/redux/reducers/claims.reducer";
...
const AdminPending = () => {
  ...
  const pendingClaimsState: Claim[] = useSelector(
    (state: RootState) => state.claim.claims
  );

  const filteredData = useMemo(() => {
    const lowerQuery = searchQuery.toLowerCase();
    return pendingClaimsState.filter(
      (claim) =>
  ...
  }, [searchQuery, pendingClaimsState]);
  ...
  const handleRowClick = (claimId?: string) => {
    if (claimId) {
      navigate(`/admin/claims/${claimId}`, {
        state: { from: "/admin/claims/pending" },
      });
    }
  };
    ...
    <div className="px-2 py-4 md:px-4">
      <div className="flex flex-col p-4 rounded-lg bg-white shadow-lg">
        ...
        <div className="mb-4 text-black">
          <h2 className="text-2xl font-bold mb-2">Pending Approved</h2>
          <p>Here is a list of pending approved</p>
...
export default withAuthorization(AdminPending, allowedRoles);

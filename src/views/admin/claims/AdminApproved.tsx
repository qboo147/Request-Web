...
import { getApprovedClaims } from "@/lib/redux/reducers/claims.reducer";
...
const AdminApproved = () => {
  ...
  const approvedClaimsState: Claim[] = useSelector(
    (state: RootState) => state.claim.claims
  );

  const filteredData = useMemo(() => {
    const lowerQuery = searchQuery.toLowerCase();
    return approvedClaimsState.filter(
      (claim) =>
  ...
  }, [searchQuery, approvedClaimsState]);
  ...
  const handleRowClick = (claimId?: string) => {
    if (claimId) {
      navigate(`/admin/claims/${claimId}`, {
        state: { from: "/admin/claims/approved" },
      });
    }
  };
    ...
    <div className="px-2 py-4 md:px-4">
      <div className="flex flex-col p-4 rounded-lg bg-white shadow-lg">
        ...
        <div className="mb-4 text-black">
          ...
          <p>Here is a list of approved</p>
...
export default withAuthorization(AdminApproved, allowedRoles);

...
import { getDraftClaims } from "@/lib/redux/reducers/claims.reducer";
...
const AdminDraft = () => {
  ...
  useEffect(() => {
    dispatch(getDraftClaims());
  }, [dispatch]);

  const draftClaimsState: Claim[] = useSelector(
    (state: RootState) => state.claim.claims
  );

  const filteredData = useMemo(() => {
    const lowerQuery = searchQuery.toLowerCase();
    return draftClaimsState.filter(
      (claim) =>
...
export default withAuthorization(AdminDraft, allowedRoles);

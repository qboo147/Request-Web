...
import { deactivateStaff } from "@/lib/redux/reducers/staff.reducer";
...
const StaffInfo = () => {
  ...
  const confirmSubmission = async () => {
    try {
      await dispatch(deactivateStaff(staff.id)).unwrap();
      navigate("/config/staff");
    } catch (err: any) {
      setError(err.message || "Failed to deactivate staff. Please try again.");
    } finally {
    ...
    <div className="px-2 py-4 md:px-4">
      <div className="relative flex flex-col p-4 h-[600px] rounded bg-white shadow-lg gap-y-5">
        ...
        <div className="absolute bottom-[19px] right-[40px] flex gap-5">
          ...
          <button
            className="bg-red-600 text-white rounded-md border w-[120px] h-[50px] font-bold font-mono"
...
export default withAuthorization(StaffInfo, allowedRoles);
